 ...
 *              Added option to use the new
 * odid_wifi_build_message_pack_beacon_frame() function.
 *
...
ID_OpenDrone::ID_OpenDrone() {
  ...
  beacon_interval = 10;
    ...
    // If ODID_PACK_MAX_MESSAGES == 10, then the potential size of the beacon
    // message is > 255.
  ...
  beacon_counter = beacon_length = beacon_timestamp = beacon_seq =
  ...
  system_data->AreaCeiling = system_data->AreaFloor = -1000.0;
...
void ID_OpenDrone::init(UTM_parameters* parameters, bool ble_drone) {
  ...
  if (parameters->region < 2) {
    system_data->ClassificationType =
        (ODID_classification_type_t) parameters->region;
  }
...
void ID_OpenDrone::set_auth(uint8_t* auth, short int len, uint8_t type) {
  ...
  if (len > 16) {
    for (auth_page_count = 1;
         (auth_page_count < ODID_AUTH_MAX_PAGES) && (i < len);
         ++auth_page_count) {
...
int ID_OpenDrone::transmit(struct UTM_data* utm_data) {
    ...
    switch (phase) {
      ...
      case 36:  // Every 300 ms.
        ...
        if (utm_data->satellites >= SATS_LEVEL_2) {
          ...
          location_data->SpeedHorizontal =
              0.514444 * (float) utm_data->speed_kn;
          location_data->SpeedVertical = INV_SPEED_V;
          ...
          location_data->TimeStamp =
              (float) ((utm_data->minutes * 60) + utm_data->seconds) +
        ...
        if ((status = encodeLocationMessage(&location_enc, location_data)) ==
            ODID_SUCCESS) {
          transmit_ble((uint8_t*) &location_enc, sizeof(location_enc));
        }
        //  else if (Debug_Serial) {
        //   sprintf(text,
        //           "ID_OpenDrone::%s, encodeLocationMessage returned %d\r\n",
        //           __func__, status);
        //   Debug_Serial->print(text);
        // }
      ...
      case 2:
        ...
        if (UAS_data.BasicID[0].IDType) {
          transmit_ble((uint8_t*) &basicID_enc[0],
                       sizeof(ODID_BasicID_encoded));
        }
      ...
      case 10:
        ...
        if (UAS_data.BasicID[1].IDType) {
          transmit_ble((uint8_t*) &basicID_enc[1],
                       sizeof(ODID_BasicID_encoded));
        }
  ...
  if ((msecs - last_wifi) >= beacon_interval) {
    ...
    if (wifi_toggle ^= 1) {  // Basic IDs, operator, system and location.
      ...
      UAS_data.BasicIDValid[0] = UAS_data.BasicIDValid[1] =
          UAS_data.LocationValid = UAS_data.SystemValid =
      ...
      UAS_data.LocationValid = UAS_data.SelfIDValid = 0;
...
int ID_OpenDrone::transmit_wifi(struct UTM_data* utm_data, int prepacked) {
  ...
  if ((length = odid_wifi_build_nan_sync_beacon_frame(
           (char*) WiFi_mac_addr, buffer, sizeof(buffer))) > 0) {
  ...
  if ((length = odid_wifi_build_message_pack_beacon_frame(
           &UAS_data, (char*) WiFi_mac_addr, ssid, ssid_length, beacon_interval,
           ++beacon_counter, beacon_frame, BEACON_FRAME_SIZE)) > 0) {
  ...
  length = (prepacked > 0) ? prepacked
                           : odid_message_build_pack(&UAS_data, beacon_payload,
                                                     beacon_max_packed);
  ...
  if (Debug_Serial) {
    sprintf(text, "ID_OpenDrone::%s %d %d+%d=%d ", __func__, beacon_max_packed,
            beacon_offset, length, len2);
    Debug_Serial->print(text);
...
int ID_OpenDrone::transmit_ble(uint8_t* odid_msg, int length) {
  ...
  if (Debug_Serial) {
    ...
    sprintf(text, "%7lu %02x (%2d,%2d) .. ", millis(), len - 1, len - 1,
            length);
    Debug_Serial->print(text);
